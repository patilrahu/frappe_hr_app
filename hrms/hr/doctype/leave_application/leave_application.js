...
frappe.ui.form.on("Leave Application", {
	setup: function (frm) {
		...
		frm.set_query("additional_approvers", function () {
            return {
                filters: {
                    name: ["!=", frm.doc.leave_approver]  // Ensure additional approver is not the same as leave approver
                }
            };
        });
	...
	half_day: function (frm) {
		...
		// }
		// frm.trigger("calculate_total_days");
		if (frm.doc.half_day && frm.doc.quarterly_day) {
            frappe.msgprint(__("You cannot select both Half Day and Quarterly Day. Please select only one."));
            frappe.validated = false;
            frm.set_value('half_day', false);
        } else {
	...
	quarterly_day: function (frm) {
        if (frm.doc.half_day && frm.doc.quarterly_day) {
            frappe.msgprint(__("You cannot select both Half Day and Quarterly Day. Please select only one."));
            frappe.validated = false;
            frm.set_value('quarterly_day', false);
        } else {
            frm.trigger("calculate_total_days");
        }
    },
		...
		if (frm.doc.from_date && frm.doc.to_date && frm.doc.employee && frm.doc.leave_type) {
			...
			return frappe.call({
				...
				args: {
					...
					quarterly_day: frm.doc.quarterly_day,
