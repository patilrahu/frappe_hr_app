...
# 	quarterly_day: int | str | None = None,
...
class LeaveApplication(Document, PWANotificationsMixin):
    ...
    def validate_leave_overlap(self):
        ...
        for d in frappe.db.sql(
            ...
            if ((cint(self.quarterly_day) == 1 and (
                    flt(self.total_leave_days) == 0.25
                    or getdate(self.from_date) == getdate(d.to_date)
                    or getdate(self.to_date) == getdate(d.from_date)
                    ))):
                    total_leaves_on_half_day = self.get_total_leaves_on_quarterly_day()
                    if total_leaves_on_half_day >= 1:
                        self.throw_overlap_error(d)
            else:
                self.throw_overlap_error(d)
    ...
    def get_total_leaves_on_quarterly_day(self):
        leave_count_on_quarterly_day_date = frappe.db.sql(
            """SELECT COUNT(name) FROM `tabLeave Application`
            WHERE employee = %(employee)s
            AND docstatus < 2
            AND status IN ('Open', 'Approved')
            AND quarterly_day = 0.25
            AND from_date = %(from_date)s
            AND name != %(name)s""",
            {"employee": self.employee, "from_date": self.from_date, "name": self.name},
        )[0][0]
        return leave_count_on_quarterly_day_date * 0.25
